...
| 文档版本 | 1.2 |
| 编制说明 | 依据 GB/T 软件工程文档习惯与项目 `SPEC.md`、`README.md` 整理；更新测试覆盖状态 |
...
| TP-STU-06 | 非功能测试（性能、安全、兼容、易用） | 非功能 | 响应时间、容量、鉴权与常见攻击面、浏览器/分辨率、关键路径可理解性 | 第 3～4 周或里程碑前 | ⚠️ 已执行中高并发压测 |
| TP-STU-07 | 分布式与韧性（网关、注册发现、依赖故障） | 非功能 + 组装 | 单点服务不可用时的错误语义、重试与恢复；与一期「无分布式事务」边界一致 | 集成环境专项 | ⚠️ 已覆盖前端容错 |
| TP-STU-08 | 并发与数据一致性（学生端视角） | 压力/一致性（抽样） | 多考生同时保存代码、进入考试；Redis/MySQL 与界面一致 | 有性能基线后执行 | ⚠️ 已覆盖读并发与防抖 |
...
**覆盖状态说明**：✅ 已覆盖（自动化用例完整验证）；⚠️ 部分覆盖（部分场景或仅 API 层验证）；⚠️ 已执行中高并发压测（非功能测试已执行 20 线程压测）；⚠️ 已覆盖前端容错（分布式韧性前端测试已完成）；⚠️ 已覆盖读并发与防抖（并发一致性前端测试已完成）；❌ 未执行（尚未实施测试）。
