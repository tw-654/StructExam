...
| 接口 | P95 | P99 | 错误率 |
|------|-----|-----|--------|
| POST /api/auth/login | < 3000ms | < 5000ms | < 1% |
| GET /api/exam/list | < 200ms | < 500ms | < 0.1% |
| GET /api/exam/{id} | < 200ms | < 500ms | < 0.1% |
| GET /api/code/{exam}/{question} | < 100ms | < 300ms | < 0.1% |
...
为避免混合场景导致 P95/P99 统计失真，采用以下策略：

1. **登录专项**：独立线程组测试登录接口，配置适当超时（30秒连接/60秒响应）
2. **读链路专项**：独立线程组测试读接口，登录前置执行一次
3. **写链路专项**：独立线程组测试代码提交等写操作，使用独立考试ID避免冲突
4. **轮询专项**：独立线程组测试轮询接口

### 6.4 测试数据隔离

- 代码提交测试使用 `SetUp` 线程组动态创建独立考试
- 每线程使用独立的考试ID，避免并发冲突
- 测试完成后建议清理测试数据
